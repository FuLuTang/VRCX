...
- :chart_with_upwards_trend: **状态统计图表算法重构 (Status Chart Refactor)**
    - 彻底背弃了原版简单的“变动次数比例”，全线采用 Jirai 版独有的“**在线持续时长比例**”算法。
    - 自动识别并扣除所有离线（Offline）重叠区间。在统计好友 30 天/90 天长周期习惯时，其准确性相比原版有质的提升。

- :pencil2: **数据抓取与持久化补全 (Persistence Enhanced)**
    - **个人简介存档**：批量拉取并保存所有好友当前的个人简介，适合新设备/重新登录使用时一键存档，配合 Bio Diff 功能使用。
    - **自我数据闭环**：现在会自动记录你自己的位置移动、头像更换、灯色切换及上下线历史。让你在“个人主页”也能像查看好友一样，查阅自己完整的 VRC 社交生涯数据。
